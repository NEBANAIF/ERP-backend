...
public class AnalyticsService {
    ...
    @Transactional(readOnly = true)
    public AnalyticsDashboardResponse dashboard(LocalDate from, LocalDate to, String granularity, boolean includeSeries) {
    ...
    private List<TimeSeriesPoint> dailySeries(LocalDate from, LocalDate to) {
        ...
        for (LocalDate d = from; !d.isAfter(to); d = d.plusDays(1)) {
            ...
            double cogsDay = nz(cogsByDay.get(d));
            double expDay = nz(expByDay.get(d));
            double gp = v[0] - cogsDay;
            double np = gp - expDay;
    ...
    private List<TimeSeriesPoint> monthlySeries(LocalDate from, LocalDate to) {
        ...
        for (Object[] row : rows) {
            ...
            String key = ymKey(y, m);
            double rev = toDouble(row[2]);
            long q = toLong(row[3]);
            byYm.put(key, new double[] { rev, q });
        }
    ...
    private List<TimeSeriesPoint> hourlySeries(LocalDate day) {
        ...
        for (Sale s : sales) {
            int h = 0;
            if (s.getSaleTime() != null) {
                h = s.getSaleTime().getHour();
                if (h < 0) h = 0;
                if (h > 23) h = 23;
            }
    ...
    /** Approximate COGS for an hour from line items in that hour (accurate per sale row). */
